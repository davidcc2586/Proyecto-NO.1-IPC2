...
CREATE TABLE RutaRegular(
    ...
    FOREIGN KEY (id_sucursalInicio) REFERENCES Sucursal(id_sucursal),
...
CREATE TABLE RutaPrivada(
    ...
    latitudA DOUBLE(10,6) NOT NULL,
    longitudA DOUBLE(10,6) NOT NULL,
...
CREATE TABLE ViajeRegular(
    ...
    FOREIGN KEY (id_rutaRegular) REFERENCES RutaRegular(id_rutaRegular)
);
...
CREATE TABLE ViajeAlquiler(
    ...
    estadoViaje ENUM('PROGRAMADO', 'VIAJANDO') DEFAULT 'PROGRAMADO',
    id_detallesViaje INT,
    FOREIGN KEY (id_UsuarioContratista) REFERENCES Usuario(id_usuario),
    FOREIGN KEY (id_bus) REFERENCES Bus(id_bus),
    FOREIGN KEY (id_chofer) REFERENCES Chofer(id_chofer),
    FOREIGN KEY (id_rutaPrivada) REFERENCES RutaPrivada(id_rutaPrivada)
);
...
CREATE TABLE Transaccion(
    ...
    FOREIGN KEY (id_compraBoleto) REFERENCES Boleto(id_boleto),
    FOREIGN KEY (id_gastoCombustible) REFERENCES GastoCombustible(id_gastoCombustible),
    FOREIGN KEY (id_gastoTaller) REFERENCES GastoTaller(id_gastoTaller),
    FOREIGN KEY (id_viajeAlquiler) REFERENCES ViajeAlquiler(id_viajeAlquiler)
);